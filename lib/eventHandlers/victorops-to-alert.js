...
function setAcknowledged(alert) {
	if(['ACKED', 'RESOLVED'].includes(alert.state.currentAlertPhase) && alert.state.ack && alert.state.ack.timestamp) {
		return _.merge(alert, {
			acknowledged: true,
...
function setHostAndServiceInfo(alert) {
	return _.merge(alert, {
		incidentName: _.get(alert, 'state.name'),
		monitoringTool: _.get(alert, 'alert.monitoringTool'),
		host: _.get(alert, 'state.host'),
		service: _.get(alert, 'state.service')
	});
}

module.exports = (teams, victoropsAlert) => {
    const mappers = [
		setHostAndServiceInfo,
