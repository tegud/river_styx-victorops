...
function sendToZendesk(credentials, alert) {
	...
	request({
			...
			elasticsearchVictorOpsAlert.setZendeskId(alert.voUuid, body.ticket.id).then(function() {
				logger.logInfo('Victorops zendeskId stored to ES.', { victorOpsId: alert.voUuid, zendeskid: body.ticket.id });
			});
		});
}
