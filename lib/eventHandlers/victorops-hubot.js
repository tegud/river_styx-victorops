...
var handlers = {
	...
	'ACKNOWLEDGEMENT': function(acknowledgement) {
		logger.logInfo('Incident No.: ' + resolution + acknowledgement.INCIDENT_NAME + ' has been Ack\'ed by: ' + acknowledgement.NOTIFICATIONAUTHOR);

		elasticsearchVictorOpsAlert.acknowledge({
			...
			logger.logInfo('Incident No.: ' + acknowledgement.INCIDENT_NAME + ', acknowledged in elasticsearch');
		});
	},
	'RECOVERY': function(resolution) {
		...
		elasticsearchVictorOpsAlert.resolve({
			incidentName: resolution.INCIDENT_NAME,
			timestamp: resolution.TIMESTAMP
		}).then(function() {
			logger.logInfo('Incident No.: ' + resolution.INCIDENT_NAME + ', resolved in elasticsearch');
		});
	}
};
